...
export async function getShopifyClient(shopId) {
  try {
    // Get access token directly from memory storage
    const accessToken = getAccessToken(shopId);
    ...
    // Ensure shopId is a proper domain
...
export async function getShopFromBackend(shopId) {
  try {
    // Get shop validation status from Laravel backend API
    const validationResponse = await apiClient.getValidationStatus(shopId);

    if (validationResponse.success) {
      // Get the real access token from memory storage
      const accessToken = getAccessToken(shopId);

      if (!accessToken) {
        console.error('No access token found in storage for shop:', shopId);
        return null;
      }
